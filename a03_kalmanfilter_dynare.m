...
% output persistence;
p.b1 = 0.757; % b1 varies between 0.1 (extremely flexible) and 0.95(extremely persistent)
% policy passthrough (the impact of monetary policy on real economy); 
p.b2 = 0.238; % b2 varies between 0.1 (low impact) to 0.5 (strong impact)
% the impact of external demand on domestic output; 
p.b3 = 0.238; % b3 varies between 0.1 and 0.7
% the weight of the real interest rate and real exchange rate gaps in Monetary Conditions Index;
p.b4 = 0.702; % b4 varies from 0.3 to 0.8
% inflation persistence; 
p.a1 = 0.419; % a1 varies between 0.4 (low persistence) to 0.9 (high persistence)
% passthrough of marginal costs to inflation (the impact of rmc on inflation); 
p.a2 = 0.174; % a2 varies between 0.1 (a flat Phillips curve and a high sacrifice ratio) to 0.5 (a steep Phillips curve and a low sacrifice ratio)
% the ratio of domestic costs in firms aggregate costs
p.a3 = 0.883; % a3 varies between 0.9 (for a relatively more closed economy) to 0.5 (for a relatively more open economy)
% policy persistence; 
p.g1 = 0.455; % g1 varies from 0 (no persistence) to 0.8 ("wait and see" policy)
% policy reactiveness: the weight put on inflation by the policy-makers 
p.g2 = 0.795; % g2 has no upper limit but must be always higher then 0 (the Taylor principle)
% policy reactiveness: the weight put on the output gap by the policy-makers 
p.g3 = 0.414; % g3 has no upper limit but must be always higher then 0
% the weight of the backward-looking component
p.e1 = 0.475; % setting e1 equal to 0 reduces the equation to the simple UIP 
...
p.rho_D4L_CPI_TAR = 0.805; 
% persistence in convergence of trend variables to their steady-state levels
p.rho_DLA_Z_BAR   = 0.949;
p.rho_DLA_GDP_BAR = 0.947;
p.rho_RR_BAR      = 0.940;
p.rho_RR_RW_BAR   = 0.727;
p.rho_L_GDP_RW_GAP = 0.948;
p.rho_RS_RW      = 0.724;
p.rho_DLA_CPI_RW = 0.950;


%% READ THE MODEL
[m,p,mss] = readmodel_est(p);

% %% Set variances for Kalman filtration
p.std_SHK_L_GDP_GAP   = 0.352;
p.std_SHK_DLA_GDP_BAR = 0.399;

p.std_SHK_DLA_CPI     = 0.417;
p.std_SHK_D4L_CPI_TAR = 0.326;

p.std_SHK_L_S = 0.408;
p.std_SHK_RS  = 0.261;

p.std_SHK_RR_BAR    = 0.139;
p.std_SHK_DLA_Z_BAR = 0.144;

p.std_SHK_L_GDP_RW_GAP = 0.286;
p.std_SHK_RS_RW        = 0.174;
p.std_SHK_DLA_CPI_RW   = 0.399;
p.std_SHK_RR_RW_BAR    = 0.211;
...
dbsave(d,'results/kalm_his_dynare.csv');
save('results/kalm_his_dynare.mat', 'g');

%% Report
% full version
disp('Generating Filtration Report with Estimated Parameters from Dynare...');
...
x.series('',[p.b1*d.L_GDP_GAP{-1} -p.b2*p.b4*d.RR_GAP p.b2*(1-p.b4)*d.L_Z_GAP p.b3*d.L_GDP_RW_GAP d.SHK_L_GDP_GAP],...
...
x.publish('results/Filtration_dynare','display',false, 'cleanup=', false);
disp('Done!!!');
